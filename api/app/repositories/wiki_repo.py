...
class WikiRepo(BaseCrud[WikiPage]):
    """Wiki 页仓储：通用增删查继承自 BaseCrud，下为实体专属查询。"""

    def __init__(self) -> None:
        super().__init__(WikiPage)

    async def get_by_slug(self, session: AsyncSession, kb_id: uuid.UUID, slug: str) -> WikiPage | None:
        res = await session.execute(
            select(WikiPage).where(WikiPage.kb_id == kb_id, WikiPage.slug == slug)
        )
        return res.scalar_one_or_none()

    async def get_by_id(self, session: AsyncSession, page_id: uuid.UUID) -> WikiPage | None:
        # 保留既有"缺失返回 None"契约（等价 BaseCrud.get_by_id_or_none）
        return await self.get_by_id_or_none(session, page_id)

    async def list_by_kb(self, session: AsyncSession, kb_id: uuid.UUID) -> list[WikiPage]:
    ...
    async def upsert(
        ...
        *,
        ...
        page = await self.get_by_slug(session, kb_id, slug)
        if page is None:
    ...
    async def replace_links(
        self, session: AsyncSession, *, from_page_id: uuid.UUID, to_slugs: list[str]
    ) -> None:
    ...
    async def links_from(self, session: AsyncSession, from_page_id: uuid.UUID) -> list[PageLink]:
        res = await session.execute(
            select(PageLink).where(PageLink.from_page_id == from_page_id)
        )
        return list(res.scalars().all())

    async def backfill_link_targets(self, session: AsyncSession, *, kb_id: uuid.UUID) -> None:
        """把本 KB 内 to_slug 命中既有页 slug 的 page_links 回填 to_page_id。"""
        pages = await self.list_by_kb(session, kb_id)
    ...
    async def delete_page(self, session: AsyncSession, page_id: uuid.UUID) -> None:
    ...
    async def add_version(
        self, session: AsyncSession, page: WikiPage, edited_by: uuid.UUID | None = None
    ) -> PageVersion:
    ...
    async def list_versions(self, session: AsyncSession, page_id: uuid.UUID) -> list[PageVersion]:
        res = await session.execute(
            select(PageVersion)
    ...
    async def get_version(
        self, session: AsyncSession, page_id: uuid.UUID, version_no: int
    ) -> PageVersion | None:
        res = await session.execute(
    ...
    async def search_pages(
        self, session: AsyncSession, kb_ids: list[uuid.UUID], q: str, limit: int = 20
    ) -> list[WikiPage]:
    ...
    async def linked_pages(
        self, session: AsyncSession, from_page_ids: list[uuid.UUID]
    ) -> list[WikiPage]:
    ...
    async def list_by_kbs(self, session: AsyncSession, kb_ids: list[uuid.UUID]) -> list[WikiPage]:
        if not kb_ids:
            return []
        res = await session.execute(select(WikiPage).where(WikiPage.kb_id.in_(kb_ids)))
        return list(res.scalars().all())

    async def all_content_pages(self, session: AsyncSession) -> list[WikiPage]:
    ...
    async def counts_by_kbs(
        self, session: AsyncSession, kb_ids: list[uuid.UUID]
    ) -> dict[uuid.UUID, int]:
    ...
    async def backlinks(self, session: AsyncSession, page_id: uuid.UUID) -> list[WikiPage]:
    ...
    async def outlinks(self, session: AsyncSession, page_id: uuid.UUID) -> list[WikiPage]:
    ...
    async def stale_pages(self, session: AsyncSession, before, limit: int = 50) -> list[WikiPage]:
    ...
    async def review_due_pages(self, session: AsyncSession, before, limit: int = 50) -> list[WikiPage]:
    ...
    async def orphan_pages(self, session: AsyncSession, limit: int = 50) -> list[WikiPage]:
    ...
    async def recent(
        self, session: AsyncSession, kb_ids: list[uuid.UUID], limit: int = 8
    ) -> list[WikiPage]:
