...
class SearchMissRepo(BaseCrud[SearchMiss]):
    """搜索无果词仓储：通用增删查继承自 BaseCrud，下为实体专属查询。"""

    def __init__(self) -> None:
        super().__init__(SearchMiss)

    async def record(self, session: AsyncSession, *, user_id: uuid.UUID, query: str) -> SearchMiss:
        miss = SearchMiss(user_id=user_id, query=query)
        session.add(miss)
        return miss

    async def top(self, session: AsyncSession, *, limit: int = 50) -> list[tuple[str, int, datetime]]:
...
search_miss_repo = SearchMissRepo()
