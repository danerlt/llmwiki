...
class PromotionService:
    ...
    async def get(self, session: AsyncSession, pr_id: uuid.UUID) -> PromotionRequest | None:
        return await promotion_repo.get_by_id(session, pr_id)

    async def list_reviewable(self, session: AsyncSession, reviewer: User) -> list[PromotionRequest]:
    ...
    async def approve(self, session: AsyncSession, reviewer: User, pr_id: uuid.UUID) -> PromotionRequest:
    ...
    async def reject(
        self, session: AsyncSession, reviewer: User, pr_id: uuid.UUID, note: str | None = None
    ) -> PromotionRequest:
        pr = await promotion_repo.get_by_id(session, pr_id)
        if pr is None or pr.status != "pending":
            raise LookupError("request not found or already decided")
