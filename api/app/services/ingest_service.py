...
class IngestService:
    """文档摄入服务：解析→分析→生成页→落库→链接图→重建 index。"""

    def _failure_code(self, exc: Exception) -> str:
    ...
    async def _prune_orphans(
        self, session: AsyncSession, kb_id: uuid.UUID, sid: str, keep_slugs: set[str]
    ) -> None:
    ...
    async def ingest_source(
        ...
        *,
        ...
        try:
            ...
            await self._prune_orphans(session, src.kb_id, sid, seen_slugs)
            await session.flush()
            ...
            await source_repo.set_status(session, source_id, "failed", error=self._failure_code(exc))
            await session.commit()
