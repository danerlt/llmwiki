...
class QueryService:
    async def _prepare(self, session: AsyncSession, user: User, question: str, kb_scope):
    ...
    async def answer(
        self, session: AsyncSession, user: User, question: str, kb_scope=None, *, llm, history=None
    ) -> dict:
        prep = await self._prepare(session, user, question, kb_scope)
        if prep is None:
