...
async def test_promote_review_approve_flow(session, client):
    ...
    try:
        ...
        assert (await client.get("/api/reviews")).json()["data"] == []  # alice 看不到公司审核队列
    finally:
    ...
    try:
        ...
        assert any(x["id"] == pr_id for x in rev.json()["data"])  # admin 队列见到
        ap = await client.post(f"/api/reviews/{pr_id}/approve")
        assert ap.status_code == 200 and ap.json()["data"]["status"] == "approved"
        pages = await client.get(f"/api/kbs/{company.id}/pages")
        assert any(p["slug"] == "想法" for p in pages.json()["data"])  # 已晋升到公司 KB
    finally:
        app.dependency_overrides.pop(get_current_user, None)
