...
async def test_admin_lists_teams_and_users(client, session):
    ...
    assert r.status_code == 200 and any(t["name"] == "项目X" for t in r.json()["data"])
    r2 = await client.get("/api/users", headers={"Authorization": f"Bearer {token}"})
    assert r2.status_code == 200 and any(u["role"] == "admin" for u in r2.json()["data"])
...
async def test_admin_deactivate_blocks_user_then_activate(client, session):
    ...
    bob_tok = bob_login.json()["data"]["access_token"]
    bobh = {"Authorization": f"Bearer {bob_tok}"}
    # bob 当前可访问
    assert (await client.get("/api/me", headers=bobh)).status_code == 200
    bob_id = bob_login.json()["data"]  # noqa: F841
    bob_uid = (await client.get("/api/me", headers=bobh)).json()["data"]["id"]
    # admin 停用 bob
    ...
    new_tok = relogin.json()["data"]["access_token"]
    assert (await client.get("/api/me", headers={"Authorization": f"Bearer {new_tok}"})).status_code == 401
    # admin 不能停用自己
    me_id = (await client.get("/api/users", headers=h)).json()["data"]
    ...
    assert a.status_code == 200 and a.json()["data"]["is_active"] is True
    ok = await client.post("/api/auth/login", json={"email": "bob@x.com", "password": "pw123456"})
    assert (await client.get("/api/me", headers={"Authorization": f"Bearer {ok.json()['data']['access_token']}"})).status_code == 200
